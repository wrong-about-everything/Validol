...
public class IsBlankTest
{
    ...
    @Test
    public void failedWithNonExistentField() throws Throwable
    {
        IsBlank named =
            new IsBlank(
                new Named<>(
    ...
    public void failedWithNonBlankData(JsonElement json) throws Throwable
    {
        ...
        assertFalse(named.result().isSuccessful());
        assertEquals("vasya", named.result().name());
        assertEquals(json, named.result().value().raw());
    }
    ...
    {
        IsBlank named = new IsBlank(new Named<>("vasya", Either.right(new Absent<>())));
